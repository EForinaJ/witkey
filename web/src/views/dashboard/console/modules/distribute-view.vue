    ...
    <ElCard v-loading="loading" shadow="never">
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <div class="flex-c">
                    <ElButton @click="handleStart(detail)"
                    v-if="detail?.status == DistributeStatus.Pending"
                    size="small" type="primary" plain>开始服务</ElButton>
                    <ElButton @click="handleComplete(detail)"
                    v-if="detail?.status == DistributeStatus.InProgress"
                    size="small" type="success" plain>完成服务</ElButton>
                    <ElButton @click="handleSettlement"
                    v-if="detail?.status == DistributeStatus.Completed"
                    size="small" type="warning" plain>报单结算</ElButton>
                </div>
            </div>
        </template>
        
        <ElDescriptions>
            <ElDescriptionsItem :span="3"  label="派单时间">
                {{ detail?.createTime }}
            </ElDescriptionsItem>
...
import { fetchGetDistributeDetail, fetchPostDistributeComplete, fetchPostDistributeStart } from '@/api/distribute';
import { ApplyStatus, DistributeStatus } from '@/enums/statusEnum';
import { DistributeType } from '@/enums/typeEnum';
import { useSiteStore } from '@/store/modules/site';
import { ElMessageBox } from 'element-plus';
import DistributeSettlementModal from './distribute-settlement-modal.vue';
...
interface Emits {
    ...
    (e: 'settlement', value: number): void
    (e: 'submit'): void
}
